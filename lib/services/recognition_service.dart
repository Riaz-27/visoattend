...
class RecognitionService {
  ...
  void initialize({int numThreads = 4}) {
    ...
    try {
      ...
      dev.log('Interpreter created on GPU');
    } catch (e) {
      dev.log('Failed to create gpu delegate: $e\nrunning on cpu');
      _interpreterOptions = tfl.InterpreterOptions();
    }
  ...
  Future<void> loadModel() async {
    try {
      ...
      // dev.log('The _inputShape = ${_inputShape.toString()}');
      // _outputBuffer = TensorBuffer.createFixedSize(_outputShape, _inputType);

    } catch (e) {
      dev.log('Unable to create interpreter, Caught Exception: ${e.toString()}');
    }
  }
